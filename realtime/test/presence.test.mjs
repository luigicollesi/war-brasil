...
test("presence usa chave versionada e rejeita identidade inválida", () => {
  assert.equal(
    presenceKey(USER_A),
...
test("heartbeat escreve somente Redis e renova TTL explícito", async () => {
  ...
  assert.equal(fake.calls.set.length, 2);
  assert.deepEqual(fake.calls.set[0].options, { EX: 90 });
  assert.deepEqual(fake.calls.set[1].options, { EX: 90 });
  assert.equal(fake.calls.set[0].key, presenceKey(USER_A));
  assert.equal(fake.calls.set[0].value, "2026-09-14T03:30:00.000Z");
});
