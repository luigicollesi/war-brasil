...
if (!connectionString) {
  console.error(
    "[assets] DATABASE_URL não está configurada para validar o catálogo de dados.",
  );
  process.exit(1);
}
...
async function loadCatalogAssets() {
  ...
  if (result.rowCount === 0) {
    throw new Error("O catálogo não possui assets de dados publicáveis para validar.");
  }
  ...
  return result.rows.map((row) => {
    ...
    const objectKey = assertDiceAssetKey(row.asset_ref);
    if (seen.has(objectKey)) {
      throw new Error(`Object key duplicada no catálogo: ${objectKey}.`);
    }
    seen.add(objectKey);

    return { cosmeticId: row.id, objectKey };
  });
}
  ...
  for (let offset = 0; offset < assets.length; offset += concurrency) {
    ...
    const results = await Promise.all(
      batch.map(async (asset) => ({
        ...asset,
        metadata: await validateDiceAssetObject(config, asset.objectKey),
      })),
    );
...
try {
  ...
  console.log(
    `[assets] ${validated.length} WebPs do catálogo validados no bucket ${config.bucket}.`,
  );
  if (knownBytes > 0) {
