...
const migrationSource = readFileSync(
  "src/lib/db/migrations/managed/043-game-modes-objective-supremacy.sql",
  "utf8",
);
...
if (!databaseUrl) {
  ...
  test("043 persiste defaults de sala e congela settings no match", async () => {
    await withTemporaryDatabase(async (connectionString) => {
      ...
      try {
        assert.ok(upStart >= 0, "migration 043 precisa manter marcador -- Up Migration");
        assert.ok(migrationSql.length > 0, "migration 043 precisa possuir SQL de up");
