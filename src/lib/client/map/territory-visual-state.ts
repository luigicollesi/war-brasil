...
const RUNTIME_READY_CLASS = "territory-runtime-ready";
...
export function ensureTerritoryRuntimeStyles(document: Document) {
  ...
  // MapZoomController may bind before or after this stylesheet is installed.
  // Marking every face after the style is ready gives its class observer a
  // deterministic invalidation signal in either ordering.
  for (const path of document.querySelectorAll<SVGPathElement>(
    "#territories path.territory",
  )) {
    path.classList.add(RUNTIME_READY_CLASS);
  }
}
