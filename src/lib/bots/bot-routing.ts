...
export function bestStrategicRoute(input: {
  ...
  const starts = input.territories
    .filter((territory) => territory.ownerPlayerId === input.playerId)
  ...
  while (unvisited.size > 0) {
    ...
    for (const territoryId of unvisited) {
      ...
      if (
        ...
        (candidate === currentDistance && current !== null && territoryId < current)
      ) {
        current = territoryId;
    ...
    for (const edge of graph.get(current) ?? []) {
      ...
      const candidate =
        currentDistance +
        stepCost(territories.get(edge.to), input.playerId, edge.connection);
