...
import { CommandShell, type CommandSceneIntent } from "@/src/components/pre-game/foundation";
...
export function LobbyClient({ code }: LobbyClientProps) {
  ...
  const sceneIntent: CommandSceneIntent = {
    mode: "lobby",
    focus: "table",
    conflictLevel: startAuthorized ? 3 : allReady ? 1 : 0,
    orbitalAlignment: startAuthorized ? 1 : 0,
  };

  return (
    <CommandShell intent={sceneIntent} sectionLabel="BRIEFING" className={styles.foundationShell}>
      <div
      ...
      >
        ...
        >
          ...
          <div className={`wb-lobby-map ${styles.domainTable}`} aria-hidden="true">
            ...
            <p className={styles.authorization}>{tableStatus}</p>
          </div>
              ...
              <div>
                ...
                <p className={styles.readyDetail}>
                  {startAuthorized
                    ...
                    : readyPending
                      ? `${readyPlayers} de ${players.length} prontos · aguardando confirmação do servidor`
                ...
                {readyError ? (
                  <p className="wb-error mt-2" role="alert">
                    {readyError}
                  </p>
                ) : null}
              </div>
